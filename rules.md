...
4. **Update it as new patterns emerge** - Treat it as a living document
...
### Disaster Recovery Brisbane Website - Critical Constraints

1. **LOCAL SERVICE ONLY** (CLAUDE.md: Project Focus)
   ...
   - ✅ FOCUS: Direct local service delivery by certified professionals in Brisbane, Ipswich, Logan

2. **CREDENTIALS** (CLAUDE.md: Critical Information)
   - ✅ Phill McGurk is an **IICRC Master Restorer** ONLY
   - ❌ NO "RAI" or "RIA" certification claims
   - ❌ NO fake statistics or reviews
   - ✅ Use only verified, factual information

3. **CONTACT INFORMATION** (CLAUDE.md: Critical Information)
   - ✅ Phone: **1300 309 361**
   - ✅ Email: **admin@disasterrecovery.com.au**
   - ❌ NO "FREE" service messaging (except "free quotes" is acceptable)

4. **SERVICE AREAS** (CLAUDE.md: Target Market)
   - ✅ High net worth residential: Hamilton, Ascot, New Farm, Toowong (Brisbane), Karalee, Brookwater, Springfield Lakes (Ipswich)
   - ✅ Commercial: Brisbane, Ipswich, Logan
   - ❌ NO claims of national or interstate coverage

5. **CONTENT RULES** (CLAUDE.md: Development Guidelines)
   - ❌ NO embellished statistics
   - ❌ NO fake reviews or testimonials
   - ✅ Professional, trustworthy tone
   - ✅ Emphasize Master Restorer certification and rapid response

---

## Auto-Verification Commands

Before committing changes, I will:

```bash
# Check for prohibited content
grep -r "RAI\|RIA" --include="*.tsx" --include="*.ts" app/ components/ lib/
grep -r "national\|nationwide" --include="*.tsx" --include="*.ts" app/
grep -r "FREE services" --include="*.tsx" --include="*.ts" app/

# Verify correct contact info
grep -r "1300.*309.*361" --include="*.tsx" --include="*.ts" app/ components/
grep -r "admin@disasterrecovery.com.au" --include="*.tsx" --include="*.ts" app/ components/
...
If I catch myself doing any of the following, I will STOP and report:

- Adding contractor management features
- Adding CRM functionality
- Creating pitch deck content
- Claiming RAI/RIA certification
- Using national/interstate language
- Adding fake reviews or statistics
- Deviating from approved contact information

---

## Session Start Protocol

At the beginning of each session, I will:

1. ✅ Read `rules.md` completely
2. ✅ Read `CLAUDE.md` completely
3. ✅ State back the 3 most important constraints
4. ✅ Confirm understanding before proceeding with any task
5. ✅ Ask for clarification on any ambiguous requirements

---

**Last Updated:** 2025-11-07
**Compliance Required:** Mandatory for all tasks
**Authority:** This file overrides any conflicting general guidance
