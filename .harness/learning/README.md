# `.harness/learning/` — RA-1745 / DR-797 Training-pipeline signal

This directory is the **training-pipeline input** for the Pi-CEO autonomous
system, per the architecture in RA-1745. DR-797 is the DR-NRPG adoption of that
framework — this repo now emits the same five learning signals as the rest of
the portfolio.
...
Any agent finishing a piece of work in inference mode appends a structured row
to one of the five logs below when it spots something the system could learn
from. The training run reads all five, identifies recurring patterns
(≥ 3 entries), and promotes them.
...
- The weekly distiller can rank patterns by repo (e.g. "Prisma drift hits
  restoreassist + carsi + dr-nrpg → cross-repo skill")

## Don't stop work to write here

The mandate: if you notice something the system should learn from, append a
structured entry **and keep working**. Do not pause to reason about meta-rules.
The weekly distillation is when meta-rules get processed.

## Adoption checklist (DR-797)

- [x] Create `.harness/learning/` in the repo root
- [x] Add the five empty JSONL logs (they keep the directory tracked)
- [x] Add this README documenting the append shape and cadence
- [ ] First real signal appended by an agent finishing work in this repo
- [ ] Repo included in the weekly Saturday distillation sweep

## Tracked tickets

- RA-1745 — architecture master (training/inference framework)
- RA-1746 — RestoreAssist adoption
- DR-797 — DR-NRPG adoption (this directory)
