...
const JobTypeSelector = ({ onSelect }) => {
  const jobTypes = [
    { type: 'Water & Flood Damage', description: 'Flood restoration services', icon: 'ð§' },
    { type: 'Mould & Air Quality', description: 'Mould remediation services', icon: 'ð' },
    { type: 'Fire & Smoke Damage', description: 'Fire damage restoration', icon: 'ð¥' },
    { type: 'Storm Damage', description: 'Storm and roof damage', icon: 'ð©ï¸' },
    { type: 'Biohazard & Sewage', description: 'Hazard cleanup', icon: 'ð§ª' }
  ];
...
const JobInputForm = ({ jobType, onCalculate }) => {
  ...
  const jobInputs = {
    'Water & Flood Damage': [
      { label: 'Affected area in mÂ²', name: 'area_m2', type: 'number' },
