/**
 * NRPG — AI Front Desk configuration (SKELETON / EXAMPLE).
 *
 * Copy to `frontdesk.config.ts` and fill real values when the shared
 * `@nexus/front-desk` package lands. Ships dark until `NRPG_FRONT_DESK_ENABLED=true`.
...
export interface FrontDeskConfig {
  /** Stable project key. */
  project: string;
  /** Enable flag — reads process.env.NRPG_FRONT_DESK_ENABLED. Default off. */
  enabled: boolean;
...
export const config: FrontDeskConfig = {
  project: 'nrpg',
  enabled: process.env.NRPG_FRONT_DESK_ENABLED === 'true',
  brand: { assistantName: 'the NRPG membership desk', accent: '#000000' /* TODO brand accent */ },
  voice: { provider: 'elevenlabs', voiceId: 'TODO_NRPG_VOICE_ID', locale: 'en-AU' },
  knowledge: { kind: 'prompt', source: 'TODO — this brand\'s knowledge base' },
  channels: ['chat', 'phone-in', 'phone-out', 'voice'],
  vendor: 'elevenlabs',
  phoneNumber: null,
  tools: ['directory', 'membership crm', 'payments', 'human handoff'],
  compliance: {
    businessIdentity: 'NRPG',
